...
class LLMTranscriptionJob(models.Model):
    _name = "llm.transcription.job"
    _description = "LLM Transcription Job"
    ...
    thread_id = fields.Many2one(
        ...
        required=True,
        ondelete="cascade",
        help="Thread that this transcription job belongs to",
    )
    ...
    model_id = fields.Many2one(
        "llm.model", string="Model", required=True, help="LLM model used for transcription"
    )
    ...
    input_message_id = fields.Many2one(
        ...
        help="Message that triggered this transcription job",
    )
    output_message_id = fields.Many2one(
        "mail.message",
        string="Output Message",
        help="Message created by this transcription job",
    )
    ...
    transcription_inputs = fields.Json(
        string="Transcription Inputs", help="Input data for the transcription process"
    )
    ...
    user_id = fields.Many2one(
        ...
        required=True,
        help="User who initiated the transcription",
    )
    ...
    def _compute_name(self):
        for job in self:
            if job.thread_id and job.model_id:
                ...
                job.name = f"Transcription Job {job.id}"
    ...
    def _compute_display_name(self):
        for job in self:
            if job.name:
                ...
                job.display_name = f"Transcription Job {job.id} ({job.state})"
    ...
    def action_queue(self):
        ...
        if self.state != "draft":
            raise UserError(
                _(
                    "This transcription request has already been submitted and cannot be queued again."
                )
            )
        ...
        self.env["llm.transcription.queue"]._process_model_queue(self.model_id)

        return True

    def action_start(self):
        """Start processing the transcription job"""
        self.ensure_one()
        if self.state != "queued":
            raise UserError(
                _(
                    "This transcription request is not ready to start. "
        ...
        # Call provider to start transcription
        try:
            external_job_id = self.provider_id.create_transcription_job(self)
    ...
    def action_cancel(self):
        ...
        if self.state == "running" and self.external_job_id:
            try:
                self.provider_id.cancel_transcription_job(self)
            except Exception as e:
    ...
    def check_status(self):
        ...
        try:
            status = self.provider_id.check_transcription_job_status(self)
            return status
        except Exception as e:
    ...
    def cleanup_old_jobs(self, days=7):
        ...
        _logger.info(f"Cleaned up {count} old transcription jobs")
        return count
    ...
    def _get_transcription_context(self):
        """Get context for transcription process"""
        self.ensure_one()
        ...
        if self.transcription_inputs:
            context.update(self.transcription_inputs)

        return context

    def get_prepared_inputs(self):
        """Get transcription inputs prepared with context and templates
        ...
        Returns:
            dict: Prepared inputs ready for transcription
        """
        self.ensure_one()

        # Check if thread has prepare_transcription_inputs method (from llm_transcribe)
        if hasattr(self.thread_id, "prepare_transcription_inputs"):
            ...
            return self.thread_id.prepare_transcription_inputs(
                self.transcription_inputs or {}, attachment_ids=attachment_ids
            )
        else:
            # Fallback to raw inputs if llm_transcribe is not installed
            return self.transcription_inputs or {}
