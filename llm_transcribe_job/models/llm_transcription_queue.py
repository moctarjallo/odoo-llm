...
class LLMTranscriptionQueue(models.Model):
    _name = "llm.transcription.queue"
    _description = "LLM Transcription Queue"
    ...
    def _compute_display_name(self):
        for queue in self:
            if queue.model_id:
                ...
                queue.display_name = "Transcription Queue"
    ...
    def _compute_job_counts(self):
        for queue in self:
            ...
            queue.current_running_jobs = self.env["llm.transcription.job"].search_count(
                domain_base + [("state", "=", "running")]
            )

            # Queued jobs
            queue.queued_jobs_count = self.env["llm.transcription.job"].search_count(
                domain_base + [("state", "=", "queued")]
            )

            # Total jobs today
            today = fields.Date.today()
            queue.total_jobs_today = self.env["llm.transcription.job"].search_count(
    ...
    def _compute_performance_metrics(self):
        for queue in self:
            ...
            recent_jobs = self.env["llm.transcription.job"].search(
                [
    ...
    def _compute_recent_jobs(self):
        for queue in self:
            if queue.model_id:
                ...
                queue.recent_job_ids = self.env["llm.transcription.job"]
    ...
    def action_enable(self):
        ...
        _logger.info(f"Enabled transcription queue for model {self.model_id.name}")

    def action_disable(self):
        """Disable the queue"""
        self.ensure_one()
        self.write({"enabled": False})
        _logger.info(f"Disabled transcription queue for model {self.model_id.name}")
    ...
    def action_clear_queue(self):
        ...
        self.ensure_one()
        queued_jobs = self.env["llm.transcription.job"].search(
            [
    ...
    def action_retry_failed_jobs(self):
        ...
        self.ensure_one()
        failed_jobs = self.env["llm.transcription.job"].search(
            [
                ("model_id", "=", self.model_id.id),
                ("state", "=", "failed"),
                (
                    "retry_count",
                    "<",
                    self.env["llm.transcription.job"]._fields["max_retries"].default,
                ),
            ]
        )
    ...
    def action_view_jobs(self):
        ...
        return {
            ...
            "res_model": "llm.transcription.job",
            "view_mode": "tree,form",
    ...
    def _process_model_queue(self, model):
        ...
        # Get pending jobs
        pending_jobs = self.env["llm.transcription.job"].search(
            [
                ("model_id", "=", model.id),
                ("state", "=", "queued"),
            ],
        ...
        for job in pending_jobs:
            try:
                job.action_start()
                processed_count += 1
                _logger.info(f"Started transcription job {job.id} for model {model.name}")
            except Exception as e:
                _logger.error(f"Failed to start transcription job {job.id}: {e}")
    ...
    def _check_job_statuses(self):
        ...
        running_jobs = self.env["llm.transcription.job"].search(
            [
                ("state", "=", "running"),
    ...
    def get_queue_stats(self):
        ...
        for state in ["draft", "queued", "running", "completed", "failed", "cancelled"]:
            job_counts[state] = self.env["llm.transcription.job"].search_count(
                [
                    ("model_id", "=", self.model_id.id),
                    ("state", "=", state),
                ]
